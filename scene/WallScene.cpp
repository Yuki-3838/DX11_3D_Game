...
void WallScene::update(uint64_t deltatime)
{
...
void WallScene::draw(uint64_t deltatime)
{
	m_camera.Draw();
...
void WallScene::init()
{
	...
	m_walls.reserve(INITIAL_WALLNUM);
	for (int wcnt = 0; wcnt < INITIAL_WALLNUM; wcnt++) {
		std::unique_ptr<wall> newWall = std::make_unique<wall>(this);
		newWall->init();
		newWall->setPosition(
			Vector3(
				static_cast<float>(rnd.uniformReal(-200, 200)),
				0,
				static_cast<float>(rnd.uniformReal(-200, 200))
			)
		);
		newWall->setheight(
			static_cast<float>(rnd.uniformReal(50, 100)));
		newWall->setwidth(
			static_cast<float>(rnd.uniformReal(100, 200)));
		newWall->setangleY(static_cast<float>(rnd.uniformReal(-PI, PI)));
		newWall->calcEqation();
		m_walls.push_back(std::move(newWall));
	}

	m_enemies.reserve(INITIAL_ENEMYNUM);
	for (int ecnt = 0; ecnt < INITIAL_ENEMYNUM; ecnt++) {
		Vector3 enemyPos(
			static_cast<float>(rnd.uniformReal(-250, 250)),
			0,
			static_cast<float>(rnd.uniformReal(-250, 250)));
		float enemyRotY = static_cast<float>(rnd.uniformReal(-PI, PI));
		m_enemies.push_back(createEnemyObject(this, m_player.get(), enemyPos, enemyRotY, 0.7f));
	}
	// PLAYER BS作成
	{
		CStaticMesh* mesh = MeshManager::getMesh<CStaticMesh>("car000.x");
...
void WallScene::dispose()
{

}

// 壁パラメータ調整
void WallScene::DebugWalls()
{
